...
export default async function DashboardPage({
  ...
  return (
    <div className="space-y-6">
      <section className="dashboard-fade-in relative overflow-hidden rounded-2xl border px-6 py-7" style={{ borderColor: 'var(--border)', background: 'linear-gradient(135deg, color-mix(in srgb, var(--accent) 12%, var(--surface-0)), var(--surface-0) 58%)' }}>
        ...
        <div className="relative">
          ...
          <h1 className="mt-2 text-3xl font-bold text-[var(--text-strong)]">Dashboard</h1>
              ...
              <CardContent className="p-0">
                <div className="flex items-center justify-between p-5">
                  <div>
                    ...
                    <p className="mt-1 text-3xl font-bold text-[var(--text-strong)]">{item.value}</p>
                    <p className="mt-1 text-xs text-[var(--text-muted)]">{item.subtitle}</p>
                  </div>
                  <div className="flex h-12 w-12 items-center justify-center rounded-xl" style={{ backgroundColor: `color-mix(in srgb, ${item.color} 20%, var(--surface-0))` }}>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <Card className="dashboard-rise-up" style={{ animationDelay: '140ms' }}>
          ...
          <CardContent>
            <p className="text-3xl font-bold text-[var(--text-strong)]">{formatMoney(monthlyRecurringRevenue)}</p>
        ...
        <Card className="dashboard-rise-up" style={{ animationDelay: '210ms' }}>
          <CardHeader>
            <CardTitle className="text-lg">Weekly Contract Revenue</CardTitle>
            <CardDescription>Weekly snapshots from active agreements.</CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-3xl font-bold text-[var(--text-strong)]">{formatMoney(weeklyContractRevenue)}</p>
        ...
        <Card className="dashboard-rise-up" style={{ animationDelay: '280ms' }}>
          ...
          <CardContent>
            <p className="text-xl font-semibold text-[var(--text-strong)]">{selectedLocation?.name || 'All Locations'}</p>
      ...
      <section className="grid gap-6 xl:grid-cols-[380px_minmax(0,1fr)]">
        <Card className="dashboard-rise-up" style={{ animationDelay: '120ms' }}>
          <CardHeader>
            <CardTitle className="text-xl">Occupancy Mix</CardTitle>
            ...
            <div className="flex items-center gap-5">
              <div className="relative h-36 w-36 shrink-0">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <p className="text-2xl font-bold text-[var(--text-strong)]">{formatPercent(occupancyRate)}</p>
                </div>
              </div>
        ...
        <Card className="dashboard-rise-up" style={{ animationDelay: '180ms' }}>
          <CardHeader>
            <CardTitle className="text-xl">Units by Location</CardTitle>
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
        <Card className="dashboard-rise-up" style={{ animationDelay: '220ms' }}>
          <CardHeader>
            <CardTitle className="text-xl">Contract Flow (Last 6 Months)</CardTitle>
            ...
            <div className="grid grid-cols-6 items-end gap-3">
              {monthlyContracts.map((item) => (
                <div key={item.key} className="flex flex-col items-center gap-2">
                  <div className="relative flex h-44 w-full items-end justify-center rounded-xl" style={{ backgroundColor: 'var(--surface-1)' }}>
                    <div
        ...
        <Card className="dashboard-rise-up" style={{ animationDelay: '280ms' }}>
          <CardHeader>
            ...
            <CardDescription>Jump into day-to-day operations.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-2">
            <Link href="/dashboard/locations" className="block rounded-xl border p-3 transition hover:opacity-90" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--surface-1)' }}>
              <p className="font-semibold text-[var(--text-strong)]">Locations</p>
              <p className="text-sm text-[var(--text-muted)]">Manage branch details and map areas.</p>
            </Link>
            <Link href="/dashboard/units" className="block rounded-xl border p-3 transition hover:opacity-90" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--surface-1)' }}>
              <p className="font-semibold text-[var(--text-strong)]">Units</p>
              <p className="text-sm text-[var(--text-muted)]">Track occupancy, rates, and maintenance.</p>
            </Link>
            <Link href="/dashboard/contracts" className="block rounded-xl border p-3 transition hover:opacity-90" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--surface-1)' }}>
              <p className="font-semibold text-[var(--text-strong)]">Contracts</p>
              <p className="text-sm text-[var(--text-muted)]">Review active and pending agreements.</p>
            </Link>
            <div className="rounded-xl border p-3" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--surface-1)' }}>
              <div className="flex items-center gap-2">
                <Activity className="h-4 w-4 text-[var(--text-muted)]" />
                <p className="text-sm font-medium text-[var(--text-strong)]">Live Snapshot</p>
              </div>
              <p className="mt-1 text-xs text-[var(--text-muted)]">Utilization at {formatPercent(utilizationRate)} with {maintenanceUnits} units in maintenance.</p>
            </div>
          </CardContent>
        </Card>
      </section>
        ...
        {[
          ...
          <Card key={status.label} className="dashboard-rise-up" style={{ animationDelay: `${320 + index * 45}ms` }}>
            <CardContent className="pt-6">
              ...
              <p className="mt-2 text-2xl font-bold" style={{ color: status.color }}>{status.value}</p>
            </CardContent>
          </Card>
        ))}
      </section>
